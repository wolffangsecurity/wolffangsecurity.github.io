# STIG ID: [WN11-CC-000110](https://stigaview.com/products/win11/v2r5/WN11-CC-000110/)
...
foreach ($Key in $RegistryValues.Keys) {
    ...
    if ($ConfiguredValue -eq $ExpectedValue) {
        Write-Output "Verification successful: $Key is set to $ConfiguredValue (HTTP printing disabled)."
    } else {
