# STIG ID: [WN11-CC-000060](https://stigaview.com/products/win11/v2r5/WN11-CC-000060/)
...
foreach ($Key in $RegistryValues.Keys) {
    ...
    if ($ConfiguredValue -eq $ExpectedValue) {
        Write-Output "Verification successful: $Key is set to $ConfiguredValue (Non-domain connections blocked)."
    } else {
