# STIG ID: [WN11-CC-000170](https://stigaview.com/products/win11/v2r5/WN11-CC-000170/)
...
foreach ($Key in $RegistryValues.Keys) {
    ...
    if ($ConfiguredValue -eq $ExpectedValue) {
        Write-Output "Verification successful: $Key is set to $ConfiguredValue (Microsoft accounts optional for modern style apps)."
    } else {
